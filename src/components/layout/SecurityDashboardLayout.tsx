...
import {
  Shield, Activity, AlertTriangle, FileText, LogOut, Key, Menu, ChevronLeft
} from "lucide-react";
import cnamLogo from "@/assets/cnam-logo.jpg";

const navigation = [
  { name: "Vue d'ensemble", tab: "overview", icon: Shield },
  { name: "Événements", tab: "events", icon: Activity },
  { name: "Menaces", tab: "threats", icon: AlertTriangle },
  { name: "Logs d'audit", tab: "logs", icon: FileText },
];

interface Props { children: React.ReactNode; activeTab?: string; onTabChange?: (tab: string) => void; }

export function SecurityDashboardLayout({ children, activeTab = "overview", onTabChange }: Props) {
  const [sidebarOpen, setSidebarOpen] = useState(true);
  const [showChangePassword, setShowChangePassword] = useState(false);
  const location = useLocation();
  const { profile, user, signOut } = useAuth();

  const displayName = profile ? `${profile.first_name} ${profile.last_name}` : user?.email?.split("@")[0] || "IT Security";
      ...
      <aside className={cn("flex flex-col bg-sidebar transition-all duration-300", sidebarOpen ? "w-64" : "w-20")}>
          ...
          <div className="flex items-center gap-3">
            ...
            {sidebarOpen && (
              <div className="animate-fade-in">
                <h1 className="text-lg font-bold text-sidebar-foreground">CNAM SOC</h1>
                <p className="text-xs text-sidebar-muted">Security Center</p>
              </div>
            )}
          </div>
        ...
        <nav className="flex-1 space-y-1 p-3 overflow-y-auto">
          {navigation.map((item) => {
            ...
            return (
              <button key={item.name} onClick={() => onTabChange?.(item.tab)} className={cn("w-full", isActive ? "sidebar-item-active" : "sidebar-item", !sidebarOpen && "justify-center px-2")} title={!sidebarOpen ? item.name : undefined}>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-3 rounded-lg p-1 hover:bg-muted transition-colors">
                  <Avatar className="h-10 w-10"><AvatarImage src={profile?.avatar_url || undefined} /><AvatarFallback className="bg-primary text-primary-foreground text-sm font-medium">{initials}</AvatarFallback></Avatar>
                </button>
              </DropdownMenuTrigger>
