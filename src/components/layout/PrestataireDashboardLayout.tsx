...
import {
  LayoutDashboard, Receipt, CreditCard, User, LogOut, Key, Menu, ChevronLeft
} from "lucide-react";
import cnamLogo from "@/assets/cnam-logo.jpg";

const navigation = [
  { name: "Tableau de bord", href: "/app/prestataire", icon: LayoutDashboard },
  { name: "Demandes", href: "/app/prestataire/demandes", icon: Receipt },
  { name: "Paiements", href: "/app/prestataire/paiements", icon: CreditCard },
  { name: "Mon profil", href: "/app/prestataire/profil", icon: User },
];

interface Props { children: React.ReactNode; }

export function PrestataireDashboardLayout({ children }: Props) {
  const [sidebarOpen, setSidebarOpen] = useState(true);
  const [showChangePassword, setShowChangePassword] = useState(false);
  const location = useLocation();
  const { profile, user, signOut } = useAuth();

  const displayName = profile ? `${profile.first_name} ${profile.last_name}` : user?.email?.split("@")[0] || "Prestataire";
          ...
          <div className="flex items-center gap-3">
            ...
            {sidebarOpen && (
              <div className="animate-fade-in">
                ...
                <p className="text-xs text-sidebar-muted">Espace Prestataire</p>
              </div>
            )}
          </div>
