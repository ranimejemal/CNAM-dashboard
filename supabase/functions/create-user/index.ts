...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (newUserRoles?.includes("admin") && requestingProfile?.email !== SUPER_ADMIN_EMAIL) {
      return new Response(
        JSON.stringify({ error: "Seul le super administrateur peut créer des comptes admin." }),
        { status: 403, headers: { "Content-Type": "application/json", ...corsHeaders } }
      );
    }
