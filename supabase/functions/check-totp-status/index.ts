...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (roleError || !roleData) {
      return new Response(
        JSON.stringify({ 
          requires_2fa: false, 
          enrolled: false,
          message: "2FA non requis pour ce rôle" 
        }),
    ...
    // Always require verification on every login for admins (no grace period)
    ...
    return new Response(
      JSON.stringify({ 
        ...
        enrolled: isEnrolled,
        pending: isPending,
        needs_verification: true, // Always require verification
