...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { email, first_name, last_name }: OTPRequest = await req.json();
    ...
    // - approved: user should login (do not allow re-request)
    // - pending: already in review (do not allow re-request)
    // - rejected: allow restarting the process by deleting the rejected request
    ...
    if (existingRequest) {
      if (existingRequest.status === "approved") {
        // Return 200 so the client can handle this gracefully (avoid throwing a FunctionsHttpError)
        return new Response(
          JSON.stringify({
      ...
      if (existingRequest.status === "rejected") {
        // Restart: remove previous rejected request so user can submit a new one after OTP verification
    ...
    if (!emailSent) {
      // Delete the OTP record since email wasn't sent
